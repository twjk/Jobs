...
class ConfigProvider extends ChangeNotifier {
  ...
  */
  ...
  String _generateMacFromDeviceId(String deviceId) {
    ...
    if (macParts.isNotEmpty) {
      final firstByteHex = macParts[0];
      final firstByteValue = int.tryParse(firstByteHex, radix: 16) ?? 0;

      if ((firstByteValue & 0x01) != 0) {
        final fixedFirstByte =
            (firstByteValue & 0xFE)
                .toRadixString(16)
                .padLeft(2, '0')
                .toUpperCase();
        macParts[0] = fixedFirstByte;
      }
    }
  ...
  Future<String> _getDeviceMacAddress() async {
    final deviceId = await _getSimpleDeviceId();
    print('原始设备ID: $deviceId');

    if (_isMacAddress(deviceId)) {
      print('直接使用设备ID作为MAC地址');
      return deviceId;
    }

    final generatedMac = _generateMacFromDeviceId(deviceId);
    print('生成的MAC地址: $generatedMac');
    return generatedMac;
  }
