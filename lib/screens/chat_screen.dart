...
class _ChatScreenState extends State<ChatScreen> {
  ...
  final double _minWaveHeight = 5.0;
  final double _maxWaveHeight = 30.0;
  ...
  XiaozhiServiceListener? _xiaozhiListener;
  ...
  void dispose() {
    ...
    if (_xiaozhiListener != null) {
      _xiaozhiService?.removeListener(_xiaozhiListener!);
    }
    _xiaozhiService?.dispose(); // 彻底释放资源
  ...
  Future<void> _initXiaozhiService() async {
    ...
    _xiaozhiListener = _handleXiaozhiMessage;
    _xiaozhiService?.addListener(_xiaozhiListener!);
  ...
  Future<void> _checkOtaActivation({bool force = false}) async {
    if (_otaChecked && !force) return;
    _otaChecked = true;
    final configProvider = Provider.of<ConfigProvider>(context, listen: false);
    final xiaozhiConfig = configProvider.xiaozhiConfigs.firstWhere(
      (config) => config.id == widget.conversation.configId,
    ...
    // 修正baseUrl，去除多余路径
    final otaBaseUrl = xiaozhiConfig.otaUrl.replaceAll(
      RegExp(r'/xiaozhi/ota/?$'),
      '',
    );
    final otaService = OtaService(baseUrl: otaBaseUrl);
    final result = await otaService.checkOtaStatus(xiaozhiConfig.macAddress);
    if (result.needsActivation) {
      setState(() {
        _needsActivation = true;
        _activationMessage = result.activationMessage;
      });
      _showActivationDialog(result.activationMessage, result.activationCode);
    } else {
      setState(() {
  ...
  void _showActivationDialog(String message, String? code) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      // 尝试从message中提取网址和验证码
      final urlReg = RegExp(r'(https?://[\w\.:/-]+)');
      final codeReg = RegExp(r'\b\d{6}\b');
      final urlMatch = urlReg.firstMatch(message ?? '');
      final codeMatch =
          code != null && code.isNotEmpty
              ? code
              : (codeReg.firstMatch(message ?? '')?.group(0) ?? '');
      final url = urlMatch != null ? urlMatch.group(0) : '';
      showDialog(
        context: context,
        barrierDismissible: false,
        builder: (context) {
          return AlertDialog(
            title: const Text('设备未绑定'),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('请在浏览器中访问以下网址，输入验证码完成设备绑定：'),
                ...
                if (codeMatch != null && codeMatch.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      const Text(
                ...
                if ((url == null || url.isEmpty) &&
                    (codeMatch == null || codeMatch.isEmpty))
                  SelectableText(
                    message,
            ...
            actions: [
              TextButton(
                onPressed: () async {
                  await _checkOtaActivation(force: true);
                  if (!_needsActivation) {
                          ...
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                color:
                                    _xiaozhiService?.isConnected ?? false
                                        ? Colors.green
                                        : Colors.red,
                                boxShadow: [
                                  BoxShadow(
                                    color: (_xiaozhiService?.isConnected ??
                                                false
                                            ? Colors.green
                                            : Colors.red)
                                        .withOpacity(0.4),
